...
def SpatialCostFunction(CPDiffVals, CPStart, TimeIndexArray, CPArray, CorrelationTuple):
	...
	for index in range(len(CPDiffVals)-1):
		...
		Slope = 0.5*(float(1)/dX)*(CorrelationTuple[CPIndex][TimeIndexArray[index]] - CorrelationTuple[CPIndex][TimeIndexArray[index]])
		TemporalCost = TemporalCost + CPDiffVals[index]*CPDiffVals[index+1]*(CorrelationTuple[CPIndex][TimeIndexArray[index]] + Slope*(TempCPVals[index+1] - CPArray[CPIndex]))
...
def CostFunction(InputTuple, CPStart, NumTimeAlloc, TimeArray, CPArray, CorrelationTuple):
	...
	for index in range(len(CPDiffVals)-1):
		...
		SpatialCost = SpatialCost + CPDiffVals[index+1]*CPDiffVals[index+1]*(CorrelationTuple[CPIndex][0] + SpaceSlope*(TempCPVals[index+1] - CPArray[CPIndex]))
...
def Driver_PreRead_Brute(Iterations,NumCPVals,TotalTime,CPVals,LagTime,CorrelationMesh): 			
	...
	for index in range(Iterations):
		...
		for index2 in range(len(TimeAlloc)):
			TimeIndexArray.append(FindIndex(LagTime,TimeAlloc[index2]))
		...
		OptimalResult = scipy.optimize.minimize(SpatialCostFunction, CPDiff_Tuple, args=SpaceParameter_Tuple, method="SLSQP", bounds=Bnds_Space, constraints=Cons_Space)
